...
public class Parser
{
    ...
    {
        _logger.Log(ActivityLogLevel.Info, "Parser.Perform", "script lines Num: " + listScriptLineTokens.Count.ToString());
        _listVar.Clear();
        ...
        if (res)
        {
            _logger.Log(ActivityLogLevel.Info, "Parser.Perform", "Instr count: " + program.ListInstr.Count().ToString());

            // ok, no error
            return true;
        }

        _logger.LogError("Parser.Perform", "Finished with error(s), count: " + result.ListError.Count().ToString());
        return false;
    }
    ...
    private bool PerformTokensLineByLine(Result result, List<InstrNameObject> listVar, List<ScriptLineTokens> listScriptLineTokens, Program program)
    {
        ...
        while (true)
        {
            ...
            {
                _logger.Log(ActivityLogLevel.Debug, "Parser.PerformTokensLineByLine", "End Of line reached, Num: " + currLineTokensIndex.ToString() + ", Line: " + currLineTokens.ScriptLine);
            ...
            _logger.Log(ActivityLogLevel.Trace, "Parser.PerformTokensLineByLine" , "Line: " + currLineTokens.ScriptLine + ", Token: " + currToken.Value);
