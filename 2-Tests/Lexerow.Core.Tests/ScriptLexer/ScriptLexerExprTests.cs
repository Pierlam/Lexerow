...
/// </summary>
[TestClass]
public class ScriptLexerExprTests
{
    /// <summary>
    ...
    [TestMethod]
    public void ParseVarSystDolDateFormatOk()
    {
        Script script = TestTokensBuilder.CreateScript("$DateFormat=\"yyyy-mm-dd\"");
        ...
        var lineTokens = listlineTokens[0];
        Assert.AreEqual(3, lineTokens.ListScriptToken.Count);

        int idx = 0;
        Assert.IsTrue(TestTokensHelper.TestSystName(lineTokens, idx++, "$DateFormat"));
        Assert.IsTrue(TestTokensHelper.TestSep(lineTokens, idx++, "="));
        Assert.IsTrue(TestTokensHelper.TestString(lineTokens, idx++, "\"yyyy-mm-dd\""));
    }
