...
public class ProgramExecutor
{
    ...
    public bool Exec(Result result, Program program)
    {
        ...
        _logger.Log(ActivityLogLevel.Info, "ProgramExecutor.Exec.Start", "Name: " + program.Script.Name + ", Instr Count: " + program.ListInstr.Count.ToString());
        ...
        if (!res)
            _logger.LogError("ProgramExecutor.Exec", result.ListError[0]);
        else
            _logger.Log(ActivityLogLevel.Info, "ProgramExecutor.Exec", "Elapsed time: " + elapsedTime);
        return res;
    }
